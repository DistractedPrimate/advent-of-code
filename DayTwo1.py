...
formated = input.replace(':',"").replace("-", " ").replace("\n", " ").split(" ")
formated = [[formated[i:i+4]] for i in range(0, len(formated), 4)]


def countLetters(low, high, letter, password):
    count = 0
    for element in password:
        if element == letter:
            count += 1

    if count > int(high) or count < int(low):
        return 0

    return 1


print(sum([countLetters(element[0][0], element[0][1], element[0][2], element[0][3]) for element in formated]))
